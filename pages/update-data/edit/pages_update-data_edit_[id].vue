...
const hasExistingAttachments = computed(() => {
  ...
  console.log('🔍 Debug hasExistingAttachments:', {
    documentCount: documentCount.value,
    hasDocuments: documentCount.value > 0,
      ...
      if (!originalDataHasContent) {
        ...
        if (activeTab.value === 'address') {
          ...
          if (Object.keys(alternativeOriginalData).length > 0) {
            ...
            // If still no data, trigger loading only if not already loading
            if (!isLoadingBasicInfo.value && !basicInfoLoaded.value) {
    ...
    if (DEMO_MODE) {
      ...
      try {
        ...
        const mapRequestTypeToTab = (type) => {
          ...
          return m[type] || 'basic-information'; // Default to basic-information if type not found
        }
        ...
        if (apiReq.old_data && Object.keys(apiReq.old_data).length > 0) {
          ...
          if (hasNullValues) {
            ...
            // Load fresh employee data to get the real original values
            try {
          ...
          // Force load original employee data for proper comparison
          try {
            await loadBasicInformation();
          } catch (error) {
          }
...
const handleSaveChanges = async () => {
  ...
  try {
    ...
    // Call PUT API directly
    const response = await $fetch(`/api/proxy/employee/change-request/${requestId.value}`, {
      method: 'PUT',
      body: payload
    });
...
const calculateChanges = () => {
  ...
  if (isDraftStatus || isRejectedStatus) {
    ...
    Object.keys(draftChanges).forEach(tabKey => {
      const changeData = draftChanges[tabKey];
...
const processApiData = async () => {
  ...
  // For need revision/rejected status: merge old_data (original) with new_data (changes)
    ...
    const mapRequestTypeToTab = (type) => {
      ...
      return m[type] || activeTab.value || 'basic-information';
    };
